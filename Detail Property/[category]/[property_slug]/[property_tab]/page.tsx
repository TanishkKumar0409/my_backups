...
const Property = () => {
  ...
  const tabs = useMemo(
    () => [
      ...
      { id: "reviews", label: "Reviews", icon: LuStar, show: true },
      {
        id: "coupons",
        label: "Coupons",
        icon: LuBadgePercent,
  ...
  // ✅ Only check tabs after property is loaded
  const validTabs = tabs.filter((t) => t.show).map((t) => t.id);
  const activeTab = validTabs.includes(property_tab || "")
    ? property_tab
    : "overview";
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* {(property?.uniqueId, JSON.stringify(property))} */}
      <TabNavigation
        tabs={tabs}
        property={property}
        getCategoryById={getCategoryById}
